...
f1_score_page = st.Page("apps/f1_score.py", title="F1-Score", icon="📈")
citation_agreement_page = st.Page("apps/citation_agreement.py", title="Citation Agreement", icon="🔗")
reproducibility_page = st.Page("apps/reproducibility.py", title="Reproducibility", icon="🔁")
interrater_agreement_page = st.Page("apps/interrater_agreement.py", title="Inter-rater Agreement", icon="🤝")
relationship_agreement_page = st.Page("apps/relationship_agreement.py", title="Relationship Agreement", icon="🕸️")

# New tools get appended to this list as they're built -- nothing else in
# this file needs to change.
pg = st.navigation([
    home_page, f1_score_page, citation_agreement_page, reproducibility_page,
    interrater_agreement_page, relationship_agreement_page,
])
pg.run()
