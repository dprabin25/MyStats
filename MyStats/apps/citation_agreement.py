...
citation of each configured type -- PMID, UniProt accessions, and
ImmuneXpresso mentions are preloaded, and more types can be added once
their format is known -- and builds a unified Citation ID x Narrative
...
with st.expander("Advanced: citation types", expanded=False):
    st.markdown(
        ...
        "group for the ID. PMID, UniProt, and ImmuneXpresso are preloaded (ImmuneXpresso has no "
        "public per-entry ID, so it's matched as a bare mention of the tool name, either spelling). "
        "Add a row for any other source once you know how it's formatted in your narratives -- "
        "label/ID order and separator (`:`, `,`, or none) don't matter as long as the pattern "
        "captures the ID."
    )
