...
with st.container(border=True):
    st.markdown("### 🧬 Known vs Unknown (Concordance)")
    st.markdown(
        "Upload each run's relationship-table output, tag it Known or Unknown, and this "
        "classifies the claimed direction (increase/decrease) for every element-pair, then "
        "scores DIRECTION CONCORDANCE between runs -- rolled up into within-Known, "
        "within-Unknown, and Known-vs-Unknown means, with a bar chart and CSV export."
    )
    st.page_link("apps/known_vs_unknown.py", label="Open this tool →", icon="🧬")
