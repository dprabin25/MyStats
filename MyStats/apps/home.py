...
with st.container(border=True):
    ...
    st.markdown(
        "For each mechanism, list the elements you're tracking and paste each run's narrative. "
        "Detects which tracked elements are mentioned together in the same sentence per run, "
        "then scores run-to-run agreement with V-measure -- a symmetric cluster-agreement score "
        "that needs no run to be \"ground truth,\" with CSV and chart export."
    )
