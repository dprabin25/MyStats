...
_DEFAULT_RUN_EDGES = [
    ...
    # "Case study V" -- unlike the first two runs, every relationship here
    # is individually cited (PMIDs kept in the row's optional note isn't
    # supported by this simple 2-column UI, so they're not entered, but
    # the relationships themselves match the narrative exactly). No
    # Anaeroglobus geminatus/Dialister invisus edge this time -- Case
    # study V's narrative never mentions either element.
    [
        ("MMP-8", "IL-1β"), ("Lactobacillus rhamnosus", "IL-1β"), ("EGF", "IL-1β"),
        ("B-cell", "IL-1β"), ("PDGF", "IL-1β"),
    ],
]
_DEFAULT_RUN_NAMES = ["", "", "Case study V"]
...
def _run_from_edges(rid, edges, name=""):
    rows = [{"id": i, "element_a": a, "element_b": b} for i, (a, b) in enumerate(edges)]
    return {"id": rid, "name": name, "rows": rows, "next_row_id": len(rows)}


def _default_runs():
    runs = [
        _run_from_edges(i, edges, name)
        for i, (edges, name) in enumerate(zip(_DEFAULT_RUN_EDGES, _DEFAULT_RUN_NAMES))
    ]
...
# The raw input, exactly as entered -- one row per (Run, Element A, Element
# B) -- kept separate from relationship_df above, which is DERIVED (deduped
# across runs, Yes/No pivoted). This is what was actually typed in, for a
# record of the input alongside the computed results.
input_rows = [
    {"Run": name, "Element A": a, "Element B": b}
    for name, rows in zip(run_names, rows_per_run) for a, b in rows
]
input_df = pd.DataFrame(input_rows, columns=["Run", "Element A", "Element B"])
...
summary_df = pd.DataFrame([
    {"Metric": "Runs compared", "Value": len(run_names)},
    {"Metric": "Valid run pairs", "Value": n_pairs},
    {"Metric": "Average Jaccard", "Value": round(avg_jaccard, 4) if avg_jaccard is not None else None},
    {"Metric": "Average Dice", "Value": round(avg_dice, 4) if avg_dice is not None else None},
    {"Metric": "Distinct relationships (any run)", "Value": len(relationship_rows)},
])
...
dl_cols[0].download_button(
    "⬇ CSV (relationship_agreement_input.csv)", data=input_df.to_csv(index=False),
    file_name="relationship_agreement_input.csv", mime="text/csv", width="stretch",
    help="Exactly what was entered -- one row per (Run, Element A, Element B), before any "
         "dedup/pivoting -- a record of the raw input alongside the computed results below.",
)
dl_cols[1].download_button(
    "⬇ CSV (relationship_agreement_summary.csv)", data=summary_df.to_csv(index=False),
    file_name="relationship_agreement_summary.csv", mime="text/csv", width="stretch",
    help="The headline numbers from section 3 above (runs compared, valid pairs, average "
         "Jaccard/Dice, distinct relationships) as a small Metric/Value table.",
)
dl_cols2 = st.columns(2)
dl_cols2[0].download_button(
