...
def build_mean_std_bar_chart(labels, means, stds, counts, title, ylabel,
                              color="#2a78d6", value_format="{:.2f}"):
    """A single-series bar chart with error bars (mean +/- std) and a
    sample-size annotation under each value label -- same publication
    style as the other MyStats charts, used for the Known-vs-Unknown
    biological-meaning concordance summary (mean direction-concordance
    per comparison type, with the std shown as an error bar and n as
    how many run-pairs went into that mean). `stds`/`counts` entries
    may be None (e.g. n=1, no std to show) -- the error bar and the
    "(n=...)" line are still drawn, just without a whisker for that
    bar."""
    plt.rcParams["font.family"] = "sans-serif"
    plt.rcParams["font.sans-serif"] = ["Arial", "Helvetica", "DejaVu Sans"]
    plt.rcParams["font.size"] = 11
    ...
    fig, ax = plt.subplots(figsize=(max(6, 1.6 * len(labels)), 4.5), dpi=300)
    fig.patch.set_facecolor("white")
    ax.set_facecolor("white")
    ...
    clean_means = [m if m is not None else 0.0 for m in means]
    bars = ax.bar(x, clean_means, width=0.55, facecolor="white", edgecolor=color,
                   linewidth=1.8, zorder=3)

    err = [s if s is not None else 0.0 for s in stds]
    ax.errorbar(x, clean_means, yerr=err, fmt="none", ecolor="#1a1a1a",
                elinewidth=1.2, capsize=5, zorder=4)

    for xi, m, s, n in zip(x, clean_means, stds, counts):
        s_val = s if s is not None else 0.0
        label = f"{value_format.format(m)}\n(n={n})"
        ax.text(xi, m + s_val + 0.04, label, ha="center", va="bottom",
                fontsize=9.5, color="#1a1a1a")

    ax.set_ylim(0, 1.18)
    ax.set_yticks(np.arange(0, 1.01, 0.2))
    ax.set_ylabel(ylabel, color="#1a1a1a", fontsize=11)
    ax.set_title(title, color="#0b0b0b", fontsize=13, fontweight="bold", pad=14)
    ax.set_xticks(x)
...
def build_single_series_bar_chart(labels, values, title, xlabel, ylabel,
                                   color="#2a78d6", value_format="{:.0f}"):
    """A single-series bar chart in the same publication style as the
    grouped chart above (same fonts/DPI/gridlines/spines/value labels),
    used for the PMID-agreement frequency distribution (Runs citing ->
    Number of PMIDs) and any future single-metric MyStats chart."""
    plt.rcParams["font.family"] = "sans-serif"
    plt.rcParams["font.sans-serif"] = ["Arial", "Helvetica", "DejaVu Sans"]
    plt.rcParams["font.size"] = 11
    plt.rcParams["pdf.fonttype"] = 42
    ...
    x = np.arange(len(labels))
    fig, ax = plt.subplots(figsize=(max(6, 1.1 * len(labels)), 4.2), dpi=300)
    fig.patch.set_facecolor("white")
    ax.set_facecolor("white")
    ...
    bars = ax.bar(x, values, width=0.55, facecolor="white", edgecolor=color,
                   linewidth=1.6, zorder=3)
    for rect, value in zip(bars, values):
        ax.text(rect.get_x() + rect.get_width() / 2, rect.get_height() + max(values) * 0.02,
                value_format.format(value), ha="center", va="bottom", fontsize=9, color="#1a1a1a")

    ax.set_ylabel(ylabel, color="#1a1a1a", fontsize=11)
    ax.set_xlabel(xlabel, color="#1a1a1a", fontsize=11)
    ax.set_title(title, color="#0b0b0b", fontsize=13, fontweight="bold", pad=14)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, color="#1a1a1a", fontsize=10)
    ax.tick_params(axis="y", colors="#1a1a1a", labelsize=10)
