...
# types" panel (e.g. Immunoxpresso) once you know that source's format --
# nothing here needs to be hardcoded for a new type to be supported.
...
def extract_citations(text, citation_types):
    ...
    for ct in citation_types:
        ...
        for match in matches:
            ...
            if not ident:
                continue
            # UniProt accessions are conventionally upper-case; other
            # types are kept exactly as matched.
            found.add((name, ident.upper() if name.lower() == "uniprot" else ident))
    return found
