...
class _UnionFind:
    """Standard union-find over arbitrary hashable elements (path
    compression, no explicit rank -- the element counts here are small
    enough that this is plenty fast). Registering an element via find()
    before it's ever union()'d is what lets a size-1 group ("Unpaired")
    become its own singleton root correctly."""
...
def build_labels(groups, tracked_elements=None):
    ...
    - Elements are merged via union-find: any two elements that appear
      together in the SAME group get merged into one cluster, and that
      merging is transitive across groups -- if group X has {A, B} and
      group Y has {B, C}, A/B/C all end up in one cluster, even though
      no single group lists all three. This matters because this tool's
      intended input is pairwise ("A pairs with B", one pair per row),
      not a strict non-overlapping partition -- the same "hub" element
      (e.g. a cytokine mentioned in every row) is expected to repeat
      across many groups, each time naming a different partner it's
      paired with. An earlier version labeled an element by only the
      FIRST group it appeared in and never merged later groups that
      repeated it -- so once the hub element "belonged" to its first
      partner, every later partner silently became a false singleton
      instead of joining the hub's cluster, understating agreement
      between two runs that actually clustered everything around the
      same hub and wildly overstating agreement in other cases (two
      runs that both produce lots of these false singletons can look
      deceptively similar even when their real pairings disagree
      completely) -- verified against a real run pair where the old
      logic scored 0.88 and the correct union-find score is 0.00.
    ...
    for group in groups:
        if tracked_elements is not None:
            group = [e for e in group if e in tracked_elements]
        if not group:
            continue
        filtered_groups.append(group)
        for e in group:
            uf.find(e)  # register even a size-1 group as its own root
        for e in group[1:]:
            uf.union(group[0], e)

    # Union-find roots aren't stable/orderable on their own -- remap to
    # clean, deterministic sequential ints in first-seen order.
    root_to_label = {}
    labels = {}
    for group in filtered_groups:
        for e in group:
            ...
            labels[e] = root_to_label[root]

    if tracked_elements is not None:
        next_singleton = len(root_to_label)
...
def mechanism_reproducibility(run_names, run_labels):
    ...
    V-measure (None if too few shared elements), "n_common" (how many
    elements that score is actually based on -- a 1.0 from 2 shared
    elements is not the same claim as a 1.0 from 20, and this is what
    lets a caller show that difference instead of hiding it), and the
    element breakdown from pairwise_element_breakdown; average_v_measure
    is the mean over pairs that had a score (None if no pair qualified)."""
    pairs = []
    scores = []
